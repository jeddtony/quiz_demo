...
// EDIT TERMINALS
////////////////////////////////////////////////////////////
var edit = {show:true, option:'', xmlFile:'', layoutNum:0, answerNum:0, sortNum:0};
...
function buildEditButtons(){
	...
	$("#sortquestionlist").change(function() {
		if($(this).val() != ''){
...
function swapQuestion(con){
	var tmpArray = question_arr[edit.sortNum];
	var tmpXML = $(edit.xmlFile).find('item').eq(edit.sortNum).clone();
	
	edit.sortNum = Number(edit.sortNum);
	if(con){
		if(edit.sortNum+1 < question_arr.length){
			question_arr[edit.sortNum] = question_arr[edit.sortNum+1];
			question_arr[edit.sortNum+1] = tmpArray;
...
function scrollSelectTo(num){
	$('#sortquestionlist').prop("selectedIndex", edit.sortNum);
	var $s = $('#sortquestionlist');
...
/*!
 * 
 * BUILD QUESTION DROPDOWN - This is the function that runs to build question dropdown
 * 
 */
function buildQuestionDD(){
	$('#questionslist').empty();
	$('#sortquestionlist').empty();
	
	for(n=0;n<question_arr.length;n++){
		$('#questionslist').append($("<option/>", {
			value: n,
			text: 'Question '+(n+1)+' : ('+question_arr[n].question+')'
		}));
		
		$('#sortquestionlist').append($("<option/>", {
			value: n,
			text: (n+1)+' : ('+question_arr[n].question+')'
		}));
	}	
}
